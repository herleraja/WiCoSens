        ...
        while True:
            ...
            for i in np.arange(4, 52, 4):
                frame = np.append(frame,
                                  colorSpaceConversionFunction_HSV(float(dt[i]), float(dt[i + 1]), float(dt[i + 2]),
                                                                   float(dt[i + 3])))
            ...
            for i in np.arange(4, 52, 4):
                frame = np.append(frame,
                                  colorSpaceConversionFunction_XYZ(float(dt[i]), float(dt[i + 1]), float(dt[i + 2]),
                                                                   float(dt[i + 3])))

            frame = frame.reshape(1, 36)

            frame_bottom = np.concatenate((frame_bottom, np.append(frame[:, 0:6], frame[:, 30:36]).reshape(1, 12)),
                                          axis=1)  # R4, R5 and L4, L5

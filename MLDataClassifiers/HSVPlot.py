...
class Scope(object):
    ...
    def update(self, values):
        ...
        h, s, v = colorSpaceUtil.rgbc2hsv(float(values[6]), float(values[7]), float(values[8]), float(values[9]))
        ...
        h, s, v = colorSpaceUtil.rgbc2hsv(float(values[10]), float(values[11]), float(values[12]), float(values[13]))
        print('{},{},{}'.format(h, s, v))
